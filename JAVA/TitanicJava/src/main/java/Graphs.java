...
public class Graphs {
    ...
    {
        Map<String, Long> mp =passengerList.stream()
                .collect(Collectors.groupingBy(Passenger::getSurvived, Collectors.counting()));

        PieChart chart = new PieChartBuilder().width (800).height (600).build ();
        Color[] sliceColors= new Color[]{new Color (180, 68, 50), new Color (130, 105, 120), new Color (80, 143, 160)};
        chart.getStyler().setSeriesColors(sliceColors);
        chart.addSeries("Survived", mp.get("0"));
        chart.addSeries("Not-Survived", mp.get("1"));
        new SwingWrapper(chart).displayChart();
    }
    ...
    public static void graphPassengerNotSurvivedSex(List<Passenger> passengerList)
    {
        ...
        Color[] sliceColors= new Color[]{new Color (180, 68, 50), new Color (130, 105, 120), new Color (80, 143, 160)};
        chart.getStyler().setSeriesColors(sliceColors);
        chart.addSeries("Survived Males", map.get("male"));
        chart.addSeries("Survived Females", map.get("female"));
        new SwingWrapper(chart).displayChart();
    }
}
